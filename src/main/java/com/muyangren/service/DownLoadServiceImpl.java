...
import com.muyangren.mapper.DownMapper;
...
public class DownLoadServiceImpl implements DownLoadService{
    ...
    public void downLoadBatch(HttpServletResponse response, Down down) {
        // --测试用例(看效果)
        downLoadBatchTest(response,down);
        // --实际项目(写了个大概模板、根据业务去整合即可)
        //downLoadBatchTestActual(response,down);
    }


    private void downLoadBatchTest(HttpServletResponse response, Down down) {
        // 测试数据 10条线程、每条线程处理25份数据
        ...
        try {
            ...
            for (int i = 1; i <= pageNum; i++) {
                executor.execute(new DownLoadThreadTest(null, countDownLatch, filePath));
            }
